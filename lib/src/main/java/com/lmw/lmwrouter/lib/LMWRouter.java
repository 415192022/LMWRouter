...
import com.lmw.lmwrouter.lib.interceptor.RouterInterceptorChain;
...
public class LMWRouter {

    //拦截器
    private List<BaseRouterInterceptor> mInterceptors;
    ...
    public static LMWRouter getInstance() {
        return LMWRouter.Holder.mInstance;
    }

    public void setInterceptors(Context context, List<BaseRouterInterceptor> interceptors) {
        Utils.init(context);
     ...
     */
    public void startActivity(@NonNull final Class<?> clz) {
        Context context = Utils.getApp();
        startActivity(context, null, context.getPackageName(), clz.getName(), new Bundle());
    }
     ...
     * @param clz The activity class.
     */
    public void startActivity(@NonNull final Class<?> clz, @NonNull Bundle bundle) {
        Context context = Utils.getApp();
        startActivity(context, null, context.getPackageName(), clz.getName(), bundle);
    }
     ...
     * @param clz The activity class.
     */
    public void startActivity(@NonNull final Class<?> clz, @NonNull Intent intent) {
        Context context = Utils.getApp();
        startActivity(context, null, context.getPackageName(), clz.getName(), intent);
    }
     ...
     * @param clz      The activity class.
     */
    public void startActivity(@NonNull final Activity activity,
                              @NonNull final Class<?> clz) {
        startActivity(activity, null, activity.getPackageName(), clz.getName(), new Bundle());
    }
    ...
    public void startActivity(@NonNull final Activity activity,
                              @NonNull final Class<?> clz,
                              @NonNull final String[] transitionNames,
                              @NonNull final View... sharedElements) {

        if (sharedElements.length != transitionNames.length) {
            throw new IllegalArgumentException("please check sharedElements length and transitionNames length");
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            for (int i = 0; i < len; i++) {
                pairs[i] = Pair.create(sharedElements[i], transitionNames[i]);
            }
        ...
        startActivity(activity, null, activity.getPackageName(), clz.getName(), bundle);
    }
     ...
     * @param clz      The activity class.
     */
    public void startActivity(@NonNull final Bundle extras,
                              @NonNull final Activity activity,
                              @NonNull final Class<?> clz) {
        startActivity(activity, extras, activity.getPackageName(), clz.getName(), new Bundle());
    }
    ...
    public void startActivity(@NonNull final Activity activity,
        ...
        startActivity(activity, null, pkg, cls, new Bundle());
    }
    ...
    public void startActivity(@NonNull final Bundle extras,
                              ...
                              @NonNull final String pkg,
                              @NonNull final String cls) {
        startActivity(activity, extras, pkg, cls, new Bundle());
    }
    ...
    public void startActivity(@NonNull final Activity activity,
                              @NonNull final Intent intent,
                              @NonNull final View... sharedElements) {
        startActivity(intent, activity, getOptionsBundle(activity, sharedElements), true);
    }
     ...
     */
    public void startActivities(@NonNull final Activity activity,
     ...
     * @return the name of launcher activity
     */
    public String getLauncherActivity(@NonNull final String pkg) {
     ...
     */
    public boolean isActivityExistsInStack(@NonNull final Activity activity) {
     ...
     * @param isLoadAnim True to use animation for the outgoing activity, false otherwise.
     */
    public void finishActivity(@NonNull final Class<?> clz, final boolean isLoadAnim) {
        List<Activity> activities = Utils.getActivityList();
     ...
     */
    public boolean finishToActivity(@NonNull final Activity activity,
     ...
     * @param clz The activity class.
     */
    public void finishOtherActivities(@NonNull final Class<?> clz) {
     ...
     *                  outgoing activity.
     */
    public void finishAllActivities(@AnimRes final int enterAnim,
                                    @AnimRes final int exitAnim) {
        List<Activity> activityList = Utils.getActivityList();
        for (int i = activityList.size() - 1; i >= 0; --i) {// remove from top
            Activity activity = activityList.get(i);
     ...
     * @param isLoadAnim True to use animation for the outgoing activity, false otherwise.
     */
    public void finishAllActivitiesExceptNewest(final boolean isLoadAnim) {
        List<Activity> activities = Utils.getActivityList();
     ...
     *                  outgoing activity.
     */
    public void finishAllActivitiesExceptNewest(@AnimRes final int enterAnim,
                                                @AnimRes final int exitAnim) {
        List<Activity> activities = Utils.getActivityList();
        for (int i = activities.size() - 2; i >= 0; i--) {
     ...
     * @return the icon of activity
     */
    public Drawable getActivityIcon(final Class<?> clz) {
     ...
     * @return the icon of activity
     */
    public Drawable getActivityIcon(final ComponentName activityName) {
        PackageManager pm = Utils.getApp().getPackageManager();
        try {
     ...
     * @return the logo of activity
     */
    public Drawable getActivityLogo(final Class<?> clz) {
    ...
    private void startActivity(final Intent intent,
                               ...
                               final Bundle options) {
        startActivity(intent, context, options, false);
    }

    private void startActivity(final Intent intent,
                               final Context context,
                               final Bundle options,
                               final boolean shareTransition) {
        Docker docker = new Docker();
        docker.setContext(context);
        docker.setIntent(intent);
        docker.setOptions(options);
        docker.setShareTransition(shareTransition);

        BaseRouterInterceptor.Chain chain = new RouterInterceptorChain(mInterceptors, 0, docker);
        boolean ret = false;
        try {
            ret = chain.proceed(docker);
    ...
    private Bundle getOptionsBundle(final Context context,
                                    final int enterAnim,
                                    final int exitAnim) {
        return ActivityOptionsCompat.makeCustomAnimation(context, enterAnim, exitAnim).toBundle();
    }

    private Bundle getOptionsBundle(final Activity activity,
     ...
     */
    public boolean isActivityExists(@NonNull final String pkg, @NonNull final String cls) {
    ...
    public void startService(Class cls) {
        //不允许后台启动service
//        if (BaseApplication.Companion.getPageCount() != 0) {
        Intent intent = new Intent(Intent.ACTION_VIEW);
        Context context = Utils.getApp();
...
//        }
    ...
    private static class Holder {
        private static LMWRouter mInstance = new LMWRouter();
    }
